...
model.eval()
with torch.no_grad():
    output, point_maps = model(image_data)
    output = torch.sigmoid(output)
    output = output.cpu().numpy() > 0.5
    print(np.unique(point_maps))
    point_maps=point_maps[6]
    point_maps = F.interpolate(
        point_maps, size=(512, 512),
        mode="bilinear")

    point_maps = torch.sigmoid(point_maps)
    point_maps = point_maps.cpu().numpy() > 0.5

print(point_maps.shape)
print(point_maps)
plt.imshow(point_maps.squeeze(0).squeeze(0))
plt.show()
