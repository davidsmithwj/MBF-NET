...
parser.add_argument('--lr_seg', type=float, default=1e-4)  #0.0001
...
#     dataset = myDataset(fold=parse_config.fold, split='train', aug=parse_config.aug)
    ...
    parse_config.root_dir=r'D:\EDGE-Download\data\isis2016'
    from dataset.isic2016 import norm01, myDataset
    dataset = myDataset(split='train',root_dir=parse_config.root_dir,aug=parse_config.aug)
    dataset2 = myDataset(split='valid', root_dir=parse_config.root_dir,aug=False)
...
if parse_config.arch == 'MBF':
    if parse_config.trans == 1:
        ...
        model = MBF(1, parse_config.net_layer, parse_config.point_pred,
                    parse_config.ppl).cuda()#（1，50，1，6）
    else:
...
def train(epoch):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        if parse_config.point_pred == 1:
            output, point_maps_pre = model(data)#[b,1,512,512]
