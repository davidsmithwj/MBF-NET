...
from .BAT_Modules import BoundaryWiseAttentionGate2D, BoundaryWiseAttentionGate1D, BoundaryWiseAttentionGateAtrous2D, BoundaryWiseAttentionGateAtrous1D,CBAMLayer
...
class BoundaryAwareTransformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        bs, c, h, w = src.shape#(2,128,32,32)
        src = src.flatten(2).permute(2, 0, 1)#(1024,2,128)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)#(1024,2,128)
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)#(1,2,128)
        if mask is not None:
            mask = mask.flatten(1)#(bitchsize,h*w)

        tgt = torch.zeros_like(query_embed)#(1,2,128)
        memory, weights = self.encoder(src,
                                       src_key_padding_mask=mask,
                                       pos=pos_embed,
                                       height=h,
                                       width=w)
#memory=(1024,2,128),weights=list(6)()
        hs = self.decoder(tgt,
                          memory,
                          memory_key_padding_mask=mask,
                          pos=pos_embed,
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_pre(self,
                    ...
                    pos: Optional[Tensor] = None):
        src2 = self.norm1(src)
        q = k = self.with_pos_embed(src2, pos)#(1024,2,128)
        src2 = self.self_attn(q,
                              k,
                              value=src2,
                              attn_mask=src_mask,
    ...
    def forward(self,
        ...
        if self.normalize_before:
            ...
            if use_bag:
                ...
                return features
        features = self.forward_post(src, src_mask, src_key_padding_mask, pos)#(1024,2,128)
        if use_bag:
            b, c = features.shape[1:]
            if self.BAG_type == '1D':
                features = features.permute(1, 2, 0)
                ...
                features = features.permute(1, 2, 0).view(b, c, height, width)#(2,128,32,32)
                # features, weights = self.BAG(features)
                features, weights = self.CBAMLayer(features)
