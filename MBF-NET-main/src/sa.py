...
class SE_Block(nn.Module):  # Squeeze-and-Excitation block
    def __init__(self, in_planes):
        ...
        self.conv1 = nn.Conv2d(1280, 80, kernel_size=1)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(80, 1280, kernel_size=1)
...
class ASPP_Bottleneck(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv_end = nn.Conv2d(256, 1, 1, 1)
    ...
    def forward(self, feature_map):
        ...
        out = torch.cat([out_1x1, out_3x3_1, out_3x3_2, out_3x3_3, out_img],
                        1)  # (shape: (batch_size, 1280, h/16, w/16))
        seaspp1 = self.senet(out)  # 加入通道注意力机制
        se_feature_cat = seaspp1 * out
        result = self.conv_cat(se_feature_cat)
        result = self.conv_end(result)
